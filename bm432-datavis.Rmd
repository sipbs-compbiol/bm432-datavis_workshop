...
dfm = dfm %>% filter(timestamp > "2024-10-10")
...
<center>
<img src="images/2024_fig_1.jpg" width="100%" />
</center>

<p class="forceBreak"></p>

- Suggested improvements:
  - The lower extent of error bars is not visible in (B), (D), (G), or (E). This is a "dynamite plot," which should be avoided.
  - Bar charts should be avoided in general; a 1D scatterplot of each dataset in (D), (G), and (E) would be clearer. In (B) a line plot with concentration on the $x$-axis would have allowed direct comparison of A16 and A18 at each concentration and shown trends more clearly.
  - Pairwise statistical comparisons are fine, but we can't see whether, e.g. there is a difference between the effects of the two concentrations of A16 in (D), or between A16 and A18 in (B). A table of contrasts could be more informative.
  - in (E) the important comparisons are not next to each other, which makes comparison more difficult; place things to be compared by the reader next to each other where possible.
