...
p = p + geom_point(size=8) + xlim(0, max(figcounts$count)) + theme_minimal()
p = p + xlab("Responses") + ylab("Figure")
p
```
</center>
...
p + theme_minimal() + theme(axis.text.x = element_text(angle = 90)) + scale_x_discrete(drop=FALSE)
```
</center>
